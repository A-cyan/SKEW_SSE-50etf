...
def P_Compute(Option, K, R, term, F):
    """
    计算前三阶矩
    ...
    [p1, p2, p3]=[0, 0, 0]
    for i in range(Option_price.shape[0]):
        p1 = p1 - (1/Option_price.iloc[i,0]**2)*Option_price.iloc[i, 1]*Option_price.iloc[i, 2]
        p2 = p2 + (2/Option_price.iloc[i,0]**2)*(1-np.log(Option_price.iloc[i,0]/F))*Option_price.iloc[i,1]*Option_price.iloc[i,2]
        p3 = p3 + (3/Option_price.iloc[i,0]**2)*(2*np.log(Option_price.iloc[i,0]/F)-(np.log(Option_price.iloc[i,0]/F))**2)*Option_price.iloc[i,1]*Option_price.iloc[i,2]
    p1 = np.e ** (R*term) * p1 - (1+np.log(F/K)-F/K)
    p2 = np.e ** (R*term) * p2 + 2*np.log(K/F)*(F/K-1)+(1/2)*(np.log(K/F))**2
    p3 = np.e ** (R*term) * p3 + 3*(np.log(K/F))**2 * ((1/3)*np.log(K/F)-1+F/K)
    return [p1, p2, p3]


def SKEW(P1, P2, term1, term2):
    """
    计算SKEW指数
    :param P1:近月期权合约前三节矩
    :param P2: 远月期权合约前三阶矩
    ...
    :return:
    """
    w = (term2 - 30/365)/(term2 - term1)
    s1 = (P1[2] - 3 * P1[0] * P1[1] + 2 * (P1[0] ** 3)) / ((P1[1] - P1[0] ** 2) ** (3 / 2))
    s2 = (P2[2] - 3 * P2[0] * P2[1] + 2 * (P2[0] ** 3)) / ((P2[1] - P2[0] ** 2) ** (3 / 2))
    test1 = (P1[2] - 3 * P1[0] * P1[1] + 2 * (P1[0] ** 3))
    test2 = ((P1[1] - P1[0] ** 2) ** (3 / 2))
    s = w*s1 + (1-w)*s2
    skew = 100-10*s
    return skew
...
if __name__ == '__main__':
    ...
    print('日期循环：\n')
    data_skew = pd.DataFrame(columns=['Date', 'skew'])
    for i in tqdm(range(len(AllDate))):
        ...
        # 计算sigma
        P1 = P_Compute(Option1, K01, R1, T1_term, F1)
        P2 = P_Compute(Option2, K02, R2, T2_term, F2)
        # 计算VIX
        skew = SKEW(P1, P2, T1_term, T2_term)
        if skew < 0 :
            continue
        data_skew = data_skew.append({'Date': Now_T, 'skew': skew}, ignore_index=True)
    data_skew.to_csv('result\\skew.csv')
    a = 1